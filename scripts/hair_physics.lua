...
events.RENDER:register(function ()
	...
	if not renderer:isFirstPerson() or client:hasIrisShader() then
		if HairRenderCount >= HairRenderLimit - 1 then
			...
			--求めた平均から髪の角度を決定する。
			local hairLimit = {{-60, 80}, {-60, 60}, {-60, 80}, {-60, 60}, {-60, -25}}
			...
			if playerPose == "FALL_FLYING" then
				hairLimit[1] = {-40, 80}
				hairLimit[3] = {-40, 80}
				rightHair:setRot(math.min(math.max(hairLimit[1][2] - math.sqrt(VelocityAverage[1] ^ 2 + VelocityAverage[2] ^ 2) * 80, hairLimit[1][1]), hairLimit[1][2]), math.min(math.max(-VelocityAverage[1] * 20 + VelocityAverage[4] * 0.05, hairLimit[2][1]), hairLimit[2][2]), 0)
				leftHair:setRot(math.min(math.max(hairLimit[3][2] - math.sqrt(VelocityAverage[1] ^ 2 + VelocityAverage[2] ^ 2) * 80, hairLimit[3][1]), hairLimit[3][2]), math.min(math.max(VelocityAverage[1] * 20 + VelocityAverage[4] * 0.05, hairLimit[2][1]), hairLimit[2][2]), 0)
				backRibbon:setRot(hairLimit[5][2], 0, 0)
			elseif playerPose == "SWIMMING" then
				hairLimit[1] = {-40, 80}
				hairLimit[3] = {-40, 80}
				rightHair:setRot(math.min(math.max(hairLimit[1][2] - math.sqrt(VelocityAverage[1] ^ 2 + VelocityAverage[2] ^ 2) * 320, hairLimit[1][1]), hairLimit[1][2]), math.min(math.max(-VelocityAverage[1] * 80 + VelocityAverage[4] * 0.1, hairLimit[2][1]), hairLimit[2][2]), 0)
				leftHair:setRot(math.min(math.max(hairLimit[3][2] - math.sqrt(VelocityAverage[1] ^ 2 + VelocityAverage[2] ^ 2) * 320, hairLimit[3][1]), hairLimit[3][2]), math.min(math.max(VelocityAverage[1] * 80 + VelocityAverage[4] * 0.1, hairLimit[2][1]), hairLimit[2][2]), 0)
				backRibbon:setRot(hairLimit[5][2], 0, 0)
			else
				local angularVelocityAbs = math.abs(VelocityAverage[4])
				if math.floor(VelocityAverage[2] * 100 + 0.5) / 100 < 0 then
					rightHair:setRot(math.min(math.max(-VelocityAverage[1] * 120 + VelocityAverage[2] * 80 - angularVelocityAbs * 0.03, hairLimit[1][1]), hairLimit[1][2]) - lookDir.y * 90, math.min(math.max(-VelocityAverage[1] * 20 - VelocityAverage[3] * 240 + VelocityAverage[4] * 0.05 - angularVelocityAbs * 0.005, hairLimit[2][1]), hairLimit[2][2]), 0)
					leftHair:setRot(math.min(math.max(-VelocityAverage[1] * 120 + VelocityAverage[2] * 80 - angularVelocityAbs * 0.03, hairLimit[3][1]), hairLimit[3][2]) - lookDir.y * 90, math.min(math.max(VelocityAverage[1] * 20 - VelocityAverage[3] * 240 + VelocityAverage[4] * 0.05 - angularVelocityAbs * 0.005, hairLimit[2][1]), hairLimit[2][2]), 0)
					backRibbon:setRot(math.min(math.max(-VelocityAverage[1] * 160 + VelocityAverage[2] * 80 - angularVelocityAbs * 0.05, hairLimit[5][1]), hairLimit[5][2]), 0, 0)
				else
					rightHair:setRot(math.min(math.max(-VelocityAverage[1] * 120 - angularVelocityAbs * 0.03, hairLimit[1][1]), hairLimit[1][2]) - lookDir.y * 90, math.min(math.max(-VelocityAverage[1] * 20 - VelocityAverage[3] * 240 + VelocityAverage[4] * 0.05 - angularVelocityAbs * 0.005, hairLimit[2][1]), hairLimit[2][2]), 0)
					leftHair:setRot(math.min(math.max(-VelocityAverage[1] * 120 - angularVelocityAbs * 0.03, hairLimit[3][1]), hairLimit[3][2]) - lookDir.y * 90, math.min(math.max(VelocityAverage[1] * 20 - VelocityAverage[3] * 240 + VelocityAverage[4] * 0.05 - angularVelocityAbs * 0.005, hairLimit[2][1]), hairLimit[2][2]), 0)
					backRibbon:setRot(math.min(math.max(-VelocityAverage[1] * 160 - angularVelocityAbs * 0.05, hairLimit[5][1]), hairLimit[5][2]), 0, 0)
				end
			end
			HairRenderCount = 0
