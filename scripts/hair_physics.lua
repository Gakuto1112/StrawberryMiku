	...
	if HairRenderCount >= HairRenderLimit - 1 then
		...
		if not renderer:isFirstPerson() or client:hasIrisShader() then
			...
			elseif playerPose == "SWIMMING" then
				...
				local rightHairYZ = math.clamp(-VelocityAverageXWithLimit * 20 - VelocityAverageYWithLimit * 20 - VelocityAverage[3] * 90 + VelocityAverage[4] * 0.05 - angularVelocityAbs * 0.005, rotLimit[2][1], rotLimit[2][2])
				local rightHairRotX = rightHair:getRot().x
				local leftHairYZ = math.clamp(VelocityAverageXWithLimit * 20 + VelocityAverageYWithLimit * 20 - VelocityAverage[3] * 90 + VelocityAverage[4] * 0.05 - angularVelocityAbs * 0.005, rotLimit[2][1], rotLimit[2][2])
